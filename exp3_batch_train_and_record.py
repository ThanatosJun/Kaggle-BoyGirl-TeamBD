...
def summarize_comparison(run_rows: list, previous_map: dict):
    ...
    print("\n=== Comparison vs Previous Run (Exp3) ===")
    print(
...
def run_config(config_path: Path, project_root: Path, experiments_dir: Path, main_script: Path):
    ...
    text_embedding_method = cfg.get("features", {}).get("text_embedding_method")
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch train Exp3 configs and write summary metrics to experiment_record_exp3.csv"
    )
    parser.add_argument(
        "--config-glob",
        default="configs/exp3_*.yaml",
        help="Glob pattern for config files",
    )
    parser.add_argument(
        "--output-csv",
        default="experiments/experiment_record_exp3.csv",
